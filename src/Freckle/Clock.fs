...
module Clock =
    ...
    let systemUtc = Clock (fun _ -> Time.ofDateTime DateTime.UtcNow)

    ///A clock that always return the same time, mainly for testing purposes
    let alwaysAt ticks = Clock (fun _ -> ticks )

    ///Get the current time
    let now (Clock m) = async { return m () }

    let inline nowSynced (Clock m) = m ()